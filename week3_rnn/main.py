...
def setup_logger(log_dir:str):
    ...
    logger = logging.getLogger("poem_trainer")
...
def run_training():
    ...
    for epoch in range(30):
        logger.info(f"Starting epoch {epoch+1}/30")
        batches_inputs, batches_outputs = generate_batch(BATCH_SIZE, poems_vector, word_to_int)
        n_chunk = len(batches_inputs)
        logger.info(f"batch_num: {n_chunk}")
        print(f"batch nums: {n_chunk}")
        for batch in range(n_chunk):
            ...
            logger.info(f"epoch: {epoch}\tbatch number: {batch}\tloss: {loss.data.tolist()}")
            ...
            if batch % 20 ==0:
                ...
                logger.info(f"Model saved at batch {batch}")
